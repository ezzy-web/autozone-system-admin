...
function Saved({ saved }) {
    const [savedVehicles, setSavedVehicles] = React.useState(saved)


    return (<SavedPageLayout savedVehicle={savedVehicles} />);
}
...
export async function getServerSideProps(context) {
    ...
    for (var savedIndex = 0; savedIndex < (cookieSaved.length < 7 ? cookieSaved.length : 6); savedIndex++) {
